...
import {foodSummary,mealCalories,validDay,validateEstimate} from './nutrition-model.js';
...
export function createNutrition({getState,persist,render,header,icon,esc,fmt,modal,closeModal,toast}) {
  ...
  function editRow(item={name:'',portion:'',kcal:0}){
    return `<fieldset class="food-edit-row"><label>Alimento<input data-item-name value="${esc(item.name)}" maxlength="200" required></label><div class="form-row"><label>Porção<input data-item-portion value="${esc(item.portion)}" maxlength="200" required></label><label>Calorias (kcal)<input data-item-kcal type="number" value="${item.kcal}" min="0" max="20000" step="1" required aria-label="Calorias de ${esc(item.name||'novo alimento')}"></label></div><button type="button" class="text-button danger" data-food-action="remove-item">Remover alimento</button></fieldset>`;
  }
  function editMeal(meal){
    modal(`<h2 id="modal-title">Editar refeição</h2><form id="food-edit-form" data-id="${esc(meal.id)}"><label>Dia da refeição<input name="date" type="date" value="${meal.date}" max="${dateKey(new Date())}" required></label><label>O que você comeu?<textarea name="description" rows="4" maxlength="4000" required>${esc(meal.text)}</textarea></label><button type="submit" name="mode" value="recalculate" formnovalidate class="secondary wide">Recalcular pelo relato</button><p class="field-note">Recalcular usa a IA e substitui os alimentos abaixo após uma resposta válida. Para ajustar manualmente, edite os itens e salve.</p><div id="food-edit-items">${meal.items.map(editRow).join('')}</div><button type="button" class="text-button" data-food-action="add-item">+ Adicionar alimento</button><div class="food-edit-total" aria-live="polite">Total: <strong data-edit-total>${fmt(mealCalories(meal))} kcal</strong></div><button type="submit" name="mode" value="manual" class="primary wide">Salvar correção</button><button type="button" class="secondary wide" data-action="close-modal">Cancelar</button></form>`);
  }
  async function calculate(meal,description=meal.text,requestedDate=meal.date){
    if(busyId)return;
    ...
    try{
      ...
      meal.date=requestedDate;day=requestedDate;meal.text=description;meal.items=result.items;meal.message=result.message;meal.status=result.needsClarification?'question':'saved';meal.edited=false;
    }catch(error){
      if(getState()!==owner || !owner.meals.includes(meal))return;
  ...
  document.addEventListener('submit',async ev=>{
    ...
    if(ev.target.id==='food-edit-form'){
      ...
      const data=new FormData(ev.target),description=String(data.get('description')).trim(),date=String(data.get('date'));
      if(!description||description.length>4000||!validDay(date)||date>dateKey(new Date())){toast('Preencha o relato e uma data válida, até hoje.');return;}
      if(ev.submitter?.value==='recalculate'){
        if(!key){toast('Configure sua chave OpenRouter no perfil.');return;}
        closeModal();await calculate(meal,description,date);return;
      }
      const items=[...ev.target.querySelectorAll('.food-edit-row')].map(row=>({name:row.querySelector('[data-item-name]').value.trim(),portion:row.querySelector('[data-item-portion]').value.trim(),kcal:Number(row.querySelector('[data-item-kcal]').value)}));
      try{validateEstimate({items,message:'',needsClarification:false});}catch{toast('Preencha pelo menos um alimento, sua porção e calorias entre 0 e 20.000.');return;}
      const previous=structuredClone(meal);
      meal.text=description;meal.date=date;meal.items=items;meal.status='saved';meal.message='Registro ajustado manualmente por você.';meal.edited=true;
      if(!await persist()){Object.assign(meal,previous);return;}
      day=date;closeModal();render();toast('Refeição atualizada no diário e no painel.');
    }
  });
  document.addEventListener('click',async ev=>{
    ...
    if(b.dataset.foodEdit){const meal=find(b.dataset.foodEdit);if(!meal||busyId)return;editMeal(meal);}
    if(b.dataset.foodAction==='add-item'){const items=document.querySelector('#food-edit-items');if(items.children.length>=40){toast('Use no máximo 40 alimentos por registro.');return;}items.insertAdjacentHTML('beforeend',editRow());items.lastElementChild.querySelector('input').focus();}
    if(b.dataset.foodAction==='remove-item'){b.closest('.food-edit-row').remove();updateEditTotal();}
  ...
  function updateEditTotal(){const output=document.querySelector('[data-edit-total]');if(output)output.textContent=fmt([...document.querySelectorAll('[data-item-kcal]')].reduce((sum,input)=>sum+(Number(input.value)||0),0))+' kcal';}
  document.addEventListener('input',ev=>{if(ev.target.matches('[data-item-kcal]'))updateEditTotal();});
